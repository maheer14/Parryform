    ...
    public class PlayerController : KinematicObject
    {
        ...
        protected override void Update()
        {
            if (controlEnabled)
            {
                ...
                else if(jumpState ==JumpState.InFlight && Input.GetButtonDown("Jump") && parryNum>0)
                {
                    StartParryWindow();
                    if (IsWithinParryWindow())
                    {
                        parrying = true;
                        jumpState = JumpState.Parrying;
                        Schedule<PlayerParry>().player = this;
                        parryNum--;
                    }
